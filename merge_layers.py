...
POBL = gpd.read_file(data + 'urban_areas_with_population_corrected.shp', 
                    columns=['NOM', 'building_count', 'total_total', 'total_homes', 'total_dones'])
TPI = gpd.read_file(data + 'URB_TPI.shp', columns=['NOM', 'TPI'])
VULN = gpd.read_file(data + 'URB_TPI.shp', columns=['NOM', 'VULNER'])  # Assumint que hi ha un camp 'VULNERAB'
...
capes_a_fusionar = [
    (WUI, ['DN_URB']), #Bo
    (AI, ['Hazard_MC']), #Bo
    (POBL, ['building_count', 'URB_TOT', 'URB_HOMES', 'URB_DONES', 'URB_VELL']),
    (TPI, ['TPI']),
    (VULN, ['VULNER'])
]
